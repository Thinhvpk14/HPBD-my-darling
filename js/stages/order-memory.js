...
import { WORDS } from "./word-memory/logic.js?v=43";
import { ROUNDS, GOAL, pickSequence, scramble, isSameOrder } from "./order-memory/logic.js?v=43";
...
export const orderMemoryStage = {
  ...
  mount(root, ctx) {
    ...
    function finish() {
      ...
      if (!stageCleared) {
        stageCleared = true;
        const next = ctx.onComplete({ rounds: GOAL });
        nextBtn.hidden = !next;
        nextBtn.onclick = () => {
